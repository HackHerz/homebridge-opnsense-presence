...
const OPNsense = require('@dennis14e/node-opnsense')
...
const init = function() {
    ...
    const net = new OpnsenseArp(this.interval, this.routerDomain, this.routerKey, this.routerSecret);
...
class OpnsenseArp extends EventEmitter {
    constructor(interval, routerDomain, routerKey, routerSecret) {
        super();
		this.client = new OPNsense.Client(routerDomain, routerKey, routerSecret, false);
    ...
    get devices() {
		return new OPNsense.Diagnostics.InterfaceClient(this.client).getArp();
    }
            ...
            .then(response => {
                try {
					if (response.status != 'success')
						throw new Error("OPNsense API Error");

					let devices = response.result
						.filter(d => !d.expired)
						.map(this.convertFromFind);
